...
private:
	...
	Mesh processMesh(aiMesh* mesh, const aiScene* scene, aiMatrix4x4 transformation)
	{
		...
		glm::mat4 _transformation = AssimpGLMHelpers::ConvertMatrixToGLMFormat(transformation);
	...
	unsigned int TextureFromEmbedded(const aiTexture* embeddedTexture)
	{
		...
		if (height == 0) {
			...
			// La textura no está comprimida, realiza el proceso anterior con STB Image o la biblioteca que prefieras
			// ...
		}
		...
		for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
		{
			...
			if (!skip)
			{
				...
				if (auto embeddedTexture = scene->GetEmbeddedTexture(str.C_Str())) {
					std::cout << "Textura embebida detectada: " << str.C_Str() << std::endl;
					std::cout << "Ancho: " << embeddedTexture->mWidth << " Alto: " << embeddedTexture->mHeight << std::endl;

					if (embeddedTexture->mWidth > 0 && embeddedTexture->pcData) {
						texture.id = TextureFromEmbedded(embeddedTexture);
					}
					else {
						std::cout << "Error: Datos de textura embebida inválidos." << std::endl;
						// Manejar el error apropiadamente, por ejemplo, podrías cargar una textura predeterminada.
						texture.id = 0;
					}
				}
				else {
